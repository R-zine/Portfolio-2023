...
export const Side = ({
  ...
  const [isHovered, setIsHovered] = useState<Record<HoverTarget, boolean>>({
    linkedin: false,
    git: false,
    reddit: false,
    email: false,
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setIsHTML(true), 1000);
  ...
  useEffect(() => {
    if (Object.values(isHovered).some(Boolean))
      document.body.classList.add("button");
    else document.body.classList.remove("button");
    return () => document.body.classList.remove("button");
  }, [isHovered]);
  ...
  const handleHoverChange = (key: HoverTarget, newValue: boolean) =>
  ...
  const handleFormSend = async (): Promise<void> => {
    const validation = validateContactForm(emailValue, messageValue);

    if (!validation.email || !validation.message) {
      setFormStatus("Enter a valid email and message.");
      return;
    }
    if (!emailKey) {
      setFormStatus("The contact form is not configured.");
      return;
    }
    ...
    try {
      await emailjs.send("service_0lnz0ab", "template_dw76dor", {
      ...
      if (isMountedRef.current) setFormStatus("Sending failed. Please try again.");
    } finally {
      ...
      >
        ...
        {isHTML && contactPhase !== 4 && (
          <>
            ...
            <group
              onPointerEnter={() => dispatch(setContactCount(3))}
              position={[-5, -2.5, 0]}
            >
              <mesh scale={0.99}>
                <RigidBody>
                ...
                >
                  <a
                    href="https://www.reddit.com/user/IvanRadevHorror"
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label="Ivan Radev on Reddit"
            ...
            <mesh
              position={[5, -2.5, 0]}
              scale={0.99}
              onPointerEnter={() => dispatch(setContactCount(5))}
            >
              <RigidBody>
                <Box args={args}>
                  <meshStandardMaterial
              ...
              >
                {formStatus && (
                  <div
                    role="status"
                    aria-live="polite"
                    style={{ color: "black", fontSize: 10, marginBottom: 4 }}
                  >
                    {formStatus}
                  </div>
                )}
