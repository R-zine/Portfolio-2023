...
interface DescriptionProps extends DelayProps {
  isQuestion?: boolean;
  href?: string;
  target?: string;
  rel?: string;
...
export const Title = styled.div<DelayProps>`
  width: 100%;
  font-size: 1.5vw;
  text-align: left;
  text-indent: 1vw;
...
export const Button = styled.button<NavigationButtonProps>`
  font-size: 2vw;
  ...
  &:hover {
    padding-left: ${(props) => (props.direction ? "0px" : "20px")};
    padding-right: ${(props) => (props.direction ? "20px" : "0px")};
    text-shadow: ${(props) =>
      props.direction ? "20px 0px 5px white" : "-20px 0px 5px white"};
  }
`;
...
export const ImageCont = styled.div<ImageContainerProps>`
  display: flex;
  flex-direction: column;
  gap: 4vh;
  align-items: center;
...
export const CursorReplace = styled.div<PositionProps>`
  position: fixed;
  top: ${(props) => props.top + "vh"};
  left: ${(props) => props.left + "vw"};
